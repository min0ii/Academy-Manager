            ...
            {!classInfo ? <NoClass /> : (
              <>
                {gradeCategories.length > 0 && (
                  <div className="flex gap-2 flex-wrap">
                    <button onClick={() => setGradeCategoryFilter(null)}
                      className={`text-xs px-3 py-1.5 rounded-full font-medium transition-colors ${!gradeCategoryFilter ? 'bg-violet-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                      전체
                    </button>
                ...
                {(() => {
                  ...
                  if (isAll) {
                    chartPoints = sortedTests
                      .filter(t => !t.absent && t.myScore !== null && t.maxScore !== null)
                      .map(t => ({
                        name: `${t.date.slice(5).replace('-', '/')}(${dayKo(t.date)}) ${t.name.slice(0, 5)}`,
                        내점수: t.maxScore! > 0 ? Math.round((t.myScore! / t.maxScore!) * 100) : null,
                        반평균: t.avgScore !== null && t.maxScore! > 0 ? Math.round((t.avgScore / t.maxScore!) * 100) : null,
                      }))
                  } else {
                    const byDate: Record<string, { my: number; max: number; avg: number; avgMax: number }> = {}
                    for (const t of sortedTests) {
                      if (t.category !== selCat) continue
                      if (!t.date) continue
                    ...
                    <div className="divide-y divide-slate-100">
                      {(showAllGrades ? [...filteredTests].reverse() : [...filteredTests].reverse().slice(0, 5)).map((t, i) => (
                        <div key={i} className="px-5 py-4">
                          ...
                          {t.examFormat !== 'pass_fail' && (t.avgScore !== null || t.classHigh !== null) && (
