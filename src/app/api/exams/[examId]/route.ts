...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ examId: string }> }) {
  ...
  if (action === 'close') {
    ...
    const { data: classStudents } = await db
      .from('class_students').select('student_id').eq('class_id', exam.class_id)
    const studentIds = (classStudents ?? []).map(cs => cs.student_id)

    // 이미 제출된 학생 확인
    const { data: submissions } = await db
      .from('exam_submissions').select('student_id').eq('exam_id', examId).eq('is_submitted', true)
    const submittedIds = new Set((submissions ?? []).map(s => s.student_id))

    // 미제출 학생들 처리 (임시저장 있으면 채점, 없으면 빈 제출)
    const { data: questions } = await db
      .from('exam_questions').select('id, score, question_type').eq('exam_id', examId)

    for (const studentId of studentIds) {
      if (submittedIds.has(studentId)) continue

      // 임시저장된 답안 조회
      const { data: draftSub } = await db
      ...
      const { data: studentAnswers } = await db
        .from('exam_student_answers').select('question_id, student_answer')
        .eq('submission_id', submissionId)
      const answerMap = new Map((studentAnswers ?? []).map(a => [a.question_id, a.student_answer]))

      // 정답 조회
      const { data: correctAnswers } = await db
        .from('exam_correct_answers').select('question_id, answer_text')
